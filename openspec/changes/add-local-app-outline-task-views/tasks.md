...
- [x] S.1 冻结范围：本 change 只做 App 内大纲候选视图、候选创建/比较/提升预览和任务板只读查看。
- [x] S.2 冻结安全边界：新增 API 全部要求 token，并且只操作当前已打开或已创建项目。
- [x] S.3 冻结创作控制边界：outline promote 默认 dry-run；任务板只读；不自动改正文、tracking、tasks 或 canon。

## P. 实现任务

- [x] P.1 用 TDD 实现 App server core 的 outline/task 方法。
...
- [x] P.2 用 TDD 接入 HTTP `/api/outlines/*` 和 `/api/tasks/board` endpoint。
