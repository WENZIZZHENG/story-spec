...
- [ ] 1.2 明确 `completedNodes` 必须继续保持字符串数组，并把 `evidence` 视为旁路字段而不是类型替换。

## 2. CLI 与 finish 流程

- [ ] 2.1 补齐发布产物中的 CLI help 可见性，确保 `task:finish` 和 `tasks:set-status` 在构建结果里可被发现。
...
- [ ] 2.2 扩展 finish 阶段的 related draft 匹配，覆盖 `content/chapter-*.md`、`content/volume*/chapter-*.md` 和短路径章节文件。
...
- [ ] 2.3 把任务验收、task board、正文与 tracking 的结果合并成单屏收尾报告。
...
- [ ] 3.2 为 tracking 增加可选 `evidence` 字段，同时保留 `completedNodes` 字符串数组兼容性和迁移校验。
...
- [ ] 4.1 跑 `npm run build`、`node dist/cli.js --help`、相关 smoke 测试和 manifest 检查，确认 CLI 产物没有漂移。
- [ ] 4.2 跑 OpenSpec 校验，确认本 change 的 proposal、design、tasks 和 specs 彼此一致。
