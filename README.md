...
当前页面已经可以保存一句灵感、吸收长文资料预览、查看核心缺口。长文资料默认只预览，不自动写入正典；只有显式勾选“写入明确表达字段”时，才复用现有 `applyConfirmed` 语义写入作者明确表达的字段，AI 候选仍保留为候选。它仍不是完整 React/Vite 前端，也不包含账号、云端、多用户、富文本编辑器、大纲任务看板或章节编辑器；更完整的创作流程仍可继续使用 CLI 和 agent 命令。自动化检查可用 `storyspec app --json --no-open` 查看启动预览。
