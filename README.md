...
它会启动只绑定 `127.0.0.1` 的本机服务，并在浏览器里打开一个零依赖工作台页面。页面包含项目抽屉、最近项目、打开/创建项目、故事档案、创作入口、当前项目状态和确认通道；`storyspec app --project <path>` 可以在启动后预打开一个 StorySpec 项目。

当前页面已经可以保存一句灵感、吸收长文资料预览、查看核心缺口、管理候选大纲和读取只读任务板。长文资料默认只预览，不自动写入正典；只有显式勾选“写入明确表达字段”时，才复用现有 `applyConfirmed` 语义写入作者明确表达的字段，AI 候选仍保留为候选。候选大纲提升默认 dry-run，不覆盖正式 `creative-plan.md`，也不自动改正文、tracking、tasks 或 canon。它仍不是完整 React/Vite 前端，也不包含账号、云端、多用户、富文本编辑器或章节编辑器；更完整的创作流程仍可继续使用 CLI 和 agent 命令。自动化检查可用 `storyspec app --json --no-open` 查看启动预览。
