...
- **写作反馈可见** - 写正文前先看约束卡、scene beat 和章节小样，正文分块推进，结束时给出验证和 tracking 待确认项
...
- 写作入口默认先给约束卡、beat 预览和章节小样，再分块写正文，最后输出验证摘要
...
- 集成 Git 进行版本控制
- 轻松回溯和比较不同版本
- 当前以本地单人项目为主；多人账号、云端项目隔离和实时协作仍是后续 Planned 路线
...
8. **AI 辅助写作** (`/storyspec-write`)
   - 先输出章节前置约束卡、3-6 条 scene beat 和章节小样
   - 章节小样只用于确认读感，不默认写入正文、tracking、tasks 或 canon
   - 完整正文按 scene、自然段组或目标字数分块推进
