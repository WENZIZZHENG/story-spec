...
Novel Writer 的目标不是用一句话替作者定完整世界观，而是把模糊创意变成可追踪、可确认、可回放的创作决策。你可以少说一点开始，但系统会把未确认内容留在澄清记录里，而不是静默写成正典。
...
```text
一句话灵感 -> novel interview -> /novel-specify preview -> /novel-plan -> /novel-tasks -> /novel-write -> novel review
```
...
- 可复制到 agent 的 `/novel-specify ...` handoff prompt。
...
再次运行 `novel interview` 或 `novel clarify` 会读取旧 `clarifications.json`：

```bash
novel clarify idea-demo --answers "core.premise=编程施法是工具，开局仍然是轻松冒险。"
```
...
`/novel-specify`、`/novel-constitution`、`/novel-plan` 和 `/novel-tasks` 都应先输出 preview。预览必须说明：
...
```bash
novel interview 001-story --premise "旧项目的一句话核心创意" --max-questions 6
```
...
- 运行 `novel status` 查看待确认决策，再运行 `novel review --json` 检查是否已有漂移。
