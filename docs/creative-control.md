...
```text
一句话灵感 -> storyspec interview -> /storyspec-specify preview -> /storyspec-plan -> /storyspec-tasks -> /storyspec-write -> storyspec review
```
...
cd my-storyspec mkdir -p stories/idea-demo
