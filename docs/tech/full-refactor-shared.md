...
- [ ] Shared-S2：Agent 文档收口。

  覆盖原 N007。并入 [full-refactor-agent-neutral.md](full-refactor-agent-neutral.md) 的 Batch A2，目标是 README 主叙事改为 agent-neutral，Codex 作为示例 integration。

- [ ] Shared-S3：World / Canon 基座。

  覆盖原 N008-N011。并入 [full-refactor-worldbuilding.md](full-refactor-worldbuilding.md) 的 Batch B0，目标是 worldbuilding ADR、WorldFact / CanonFact / SceneCard schema、world/canon 模板与 validate issue 格式。
