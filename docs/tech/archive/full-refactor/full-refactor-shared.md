...
Completed。本文保存 A/B/C 三条路线共用的验证矩阵、风险与缓解、共享批次映射和参考链接。当前 full-refactor 主批次已完成并通过大阶段收尾验证；后续增强可复用本文验证矩阵。
...
| 声音校验 | `node dist/cli.js voice:check --json` |
| 审稿面板 | `node dist/cli.js review --json` |
| 类型 preset | `node dist/cli.js preset:list --json` / `node dist/cli.js preset:doctor --json` |
...
| 对白检查 | `node dist/cli.js dialogue:check --chapter 003 --json` |
| 剧情分支 | `node dist/cli.js branch:compare <branchId> --json` |
| Promise 检查 | `node dist/cli.js promise:check --json` |
| 张力曲线 | `node dist/cli.js tension:chart --json` |
| 风格检查 | `node dist/cli.js style:lint --chapter 003 --json` |
| 编译导出 | `node dist/cli.js compile --format markdown` |
| Research 校验 | `node dist/cli.js research:check --json` |
| Feedback 任务草稿 | `node dist/cli.js feedback:to-tasks --json` |
...
- [x] Shared-S4：Workbench 基座。

  覆盖原 N012-N015。已并入 [full-refactor-workbench.md](full-refactor-workbench.md) 的 Batch C0，完成 ContextPack / DraftRecord / NarrativeTestResult schema、context pack MVP、draft/revision 布局和 narrative:test MVP。

- [x] Shared-S5：Workbench 创作扩展。

  覆盖原 C4-C6。已并入 [full-refactor-workbench.md](full-refactor-workbench.md) 的 Batch C1，完成 DialogueBeat、StoryBranch、StoryPromise、TensionPoint schema，落地 dialogue、branch、promise/tension CLI，并接入 `/plan`、`/tasks`、`/write` 与 reviewer loop。

- [x] Shared-S6：Workbench 收口能力。

  覆盖原 C7-C9。已并入 [full-refactor-workbench.md](full-refactor-workbench.md) 的 Batch C2，完成 Research Vault、Style Linter、Markdown compile/export 与 Feedback Loop，所有能力默认不改正文，并通过 smoke 覆盖 CLI 端到端路径。

## 历史归档

上一轮全面重构已完成，归档信息见 [full-refactor-archive.md](full-refactor-archive.md)。当前 full-refactor 收尾结果见 [full-refactor-completed.md](full-refactor-completed.md)。
