...
Archive。本文保存当前 full-refactor 路线中已经完成的批次，避免活跃路线图变重。新的活跃待办以 [todo-index.md](../../todo-index.md) 为准。
...
- `docs/tech/archive/decisions/agent-neutral-refactor.md` 记录 `AIPlatform` 到 `AgentIntegration` 的重构决策、非目标和兼容策略。
...
- `docs/tech/archive/decisions/plugin-entrypoint-decision.md` 决定当前继续复用 `plugins:add`，未来 `preset:add` / `extension:add` 作为薄 alias 再评估。
...
- 新增 `docs/agent-integrations.md`、`docs/agent-contract.md`，并更新当时的迁移说明和 CHANGELOG。
...
- 新增 `docs/tech/archive/completed-roadmaps/worldbuilding-quality-roadmap.md`，明确 World Bible、Canon Ledger、Entity Graph、Scene Cards 等能力的第一版边界与旧文件映射。
...
覆盖原 C0-C3 与 Shared N012-N015。

已完成：

- 新增 `ContextPack`、`DraftRecord`、`DraftIndex`、`NarrativeTestResult` 最小领域模型。
- 新项目初始化会创建 `.specify/context-packs/` 和 `build/`。
- 新增 `novel context:pack`，生成 `.specify/context-packs/*.json/.md`，每个 mustRead 都有 reason，并汇总 WorldFact、CanonFact、Scene Card 和 VoiceFingerprint 索引。
- 新增 `novel context:validate`，校验 Context Pack 的必读路径、reason、任务写入边界和过期状态。
- 新增 `novel draft:new`、`draft:list`、`draft:promote`，草稿默认写入 `stories/*/drafts/` 和 `draft-index.json`；只有 `draft:promote --yes` 才发布到 `content/<chapter>.md`。
- 新增 `novel narrative:test`，基于 Scene Card 检查 goal/conflict/outcome、reveals 和伏笔闭环；没有 Scene Card 时 fallback 到章节任务验收。
- 新增 `/context-pack` 通用命令模板；`/write` 优先读取匹配 Context Pack。
- README、CHANGELOG、Workbench 路线和 Shared 映射已同步。
...
- `npm test` 通过。
- `npm run test:smoke` 通过。
- `npm run build:commands` 已重新生成 16 个 agent command。
- `npm run check:command-manifest` 通过。
- `git diff --check` 通过。
...
- 新增 `DialogueBeat`、`StoryBranch`、`StoryPromise`、`TensionPoint` 与 `PromiseIssue` 领域模型。
- 新增 `novel dialogue:extract`、`dialogue:plan`、`dialogue:check`，对白计划输出待确认 YAML，不直接写入 canon，并校验 speaker、intent、relationshipChange 与 VoiceFingerprint。
- 新增 `novel branch:create`、`branch:list`、`branch:compare`、`branch:promote`，剧情分支默认只写 `stories/*/branches/`；compare 输出 scene、canon、promise、relationship 和 task 影响；promote 需要 `--yes` 显式确认且不静默覆盖 main content/canon。
- 新增 `spec/tracking/promises.json` 与 `tension-curve.json` 初始化模板。
- 新增 `novel promise:list`、`promise:check`、`tension:chart`，能识别长期未兑现、payoff 缺 evidence、重复建立但不推进、高张力低回报和长期低张力问题，并生成任务草稿。
- reviewer loop 已接入 promise/tension findings；`/plan`、`/tasks`、`/write` 已读取 promise/tension、dialogue 与 branch 上下文。
- README、CHANGELOG、Workbench 路线和 Shared 验证矩阵已同步。
...
- `npm test` 通过。
- `npm run test:smoke` 通过。
- `npm run build:commands` 已重新生成 agent command。
- `npm run check:command-manifest` 通过。
- `git diff --check` 通过。

### [x] Batch C2：Research / Style / Compile / Feedback 收口

覆盖原 C7-C9 与 Shared-S6。

已完成：

- 新增 `ResearchSource`、`CitationLink`、`StyleRule`、`StyleLintFinding`、`CompileChapter`、`CompileWarning` 与 `ReaderFeedback` 领域模型。
- 新项目初始化会生成 `spec/style/`、`research/` 与 `feedback/` 基础模板。
- 新增 `novel research:add`、`research:list`、`research:link`、`research:check`，Research Vault 默认不联网，支持自由 Markdown 笔记和 citation 结构校验。
- 新增 `novel style:lint`、`style:explain`，按 `spec/style/*.yaml` 规则检查正文，finding 包含 `ruleId`、`path`、`evidence`、`suggestion`，规则可设置 `severity: off` 关闭或降级。
- 新增 `novel compile --format markdown`，只写 `build/`，输出 `manuscript.md`、`manuscript.frontmatter.json`、`reports/manuscript-report.json`、字数统计和缺失章节 warning。
- 新增 `novel feedback:import`、`feedback:list`、`feedback:triage`、`feedback:to-tasks`，反馈默认不改正文，`feedback:to-tasks` 只生成待确认任务草稿，不写入 `tasks.md`。
- README、CHANGELOG、Workbench 路线、Shared 验证矩阵和 smoke 覆盖已同步。

验证口径：

- `npm run build` 已在功能薄片开发中通过。
- `npx vitest run tests/unit/manage-research.test.ts` 通过。
- `npx vitest run tests/unit/manage-style.test.ts` 通过。
- `npx vitest run tests/unit/compile-manuscript.test.ts` 通过。
- `npx vitest run tests/unit/manage-feedback.test.ts` 通过。
- `npm run build` 通过。
- `npm test` 通过。
- `npm run build:commands` 已重新生成 agent command。
- `npm run test:smoke` 通过。
...
### [x] full-refactor 大阶段收尾

覆盖当前 full-refactor 的 Agent-neutral、Worldbuilding 与 Workbench 三条主路线。

已完成：

- `full-refactor-todo.md` 状态更新为 Completed，并记录 A/B/C 主批次均已完成。
- `full-refactor-agent-neutral.md` 状态更新为 Completed，后续 `agent:remove`、`commands:build`、CommandSpec YAML 全量迁移和 Continue checks 归类为增强批次。
- `full-refactor-workbench.md` 状态更新为 Completed，后续 `context:show`、`revise`、EPUB/Pandoc 导出、Vale/textlint 接入和反馈深度联动归类为增强批次。
- `full-refactor-shared.md` 状态更新为 Completed，并保留验证矩阵供后续增强复用。
- 完整验证未暴露需要立即修复的代码缺口。

验证口径：

- `npm run verify` 通过。
- verify 覆盖 `npm run build`、`npm test`、`npm run test:coverage`、`npm run check:changes`、`npm run check:command-manifest`、`npm run build:commands`、二次 `npm run build` 和 `npm run test:smoke`。
- 单元测试：47 个文件、151 个用例通过。
- Smoke 测试：3 个文件、28 个用例通过。
