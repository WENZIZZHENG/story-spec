...
Completed。本文承接总路线 F16-F21，聚焦有趣选择质量标准、六大核心入口卡、势力入口、首轮样例脚本、创作乐趣体验验收和低负担创作体验。F16-F21 已完成；后续新体验优化应另建专题 roadmap。
...
- F18：Completed。见本文 Batch F18。
- F19：Completed。见本文 Batch F19。
- F20：Completed。见本文 Batch F20。
- F21：Completed。见本文 Batch F21。

## Batch F16：有趣选择质量标准

状态：Completed（2026-05-04）。已为澄清问题增加 `choiceImpact` 和 `InterestingChoice`，高影响候选会检查吸引力、代价、关系影响、世界影响、后续钩子和确认边界；`clarifications.md` 会渲染这些字段，当前先覆盖 `magic.rule-hardness` 作为首个高影响模板样例。
...
状态：Completed（2026-05-04）。已为 faction 高影响示例分叉增加 `powerStructure` 轻量结构，覆盖资源控制、合法性来源、获利者、受损者、公开叙事、内部裂缝、第一碰撞场景和关系钩子；`clarifications.md` 会渲染这些候选结构，schema 会 warning 缺字段；`creative:report` 会把“只有势力或反派名称”的答案标为 partial，并提示补资源控制、合法性来源和第一碰撞场景。
...
状态：Completed（2026-05-04）。已新增 `tests/fixtures/co-creation/programming-casting-first-round.json` 和 `validateFirstRoundScript`，把《编程施法》首轮体验固化为可测试契约：保存灵感、推荐能力/舞台/势力入口、展示高影响候选、支持确认/改写/拒绝/稍后决定、输出创作回声和下一步推荐，并明确禁止首轮直接写完整 `creative-plan.md` 或把未确认角色/势力写入正典。
...
状态：Completed（2026-05-04）。已新增 `evaluateCoCreationExperience` 和 `docs/tech/co-creation-experience-acceptance.md`，将体验验收拆成自动检查与人工走查：入口自由、低负担候选、有趣选择后果、创作回声、候选边界、plan 门禁、作者回应范围，以及“是否像在创造小说世界”的人工判断。自动检查已接入《编程施法》首轮脚本，退化为重表单或 plan-first 流程会产生失败项。
