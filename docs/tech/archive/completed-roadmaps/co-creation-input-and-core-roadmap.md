...
- 类型：输入解析、研究储备。
- 状态：基础版已落地，无标题长文会按关键词进入候选分组，低置信度内容不会自动写入 confirmed。
...
- 缺口：候选分组已补齐；后续可继续增强段落证据、冲突检测和“可能属于多个字段”的候选提示。
...
- 验收标准：无标题 500 字设定能给出更合理的候选分组；低置信度内容不会自动写入 confirmed。基础版已通过单测和 CLI smoke。
