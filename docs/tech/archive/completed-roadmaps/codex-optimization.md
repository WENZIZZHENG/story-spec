...
- 增加 `novel handoff`，生成断点续写上下文包，帮助 Codex 在长篇项目中快速恢复当前章节和下一任务。
- 增加 `novel tasks:board`，把 `tasks.md` 导出为本地 JSON 看板，并为每个任务生成 GitHub issue 草稿字段。
...
- `templates/agent/agent-contract.md` 作为项目级 agent 合约模板，由 `initProject` 同步为 `.specify/agent-contract.md` 和根目录 `AGENTS.md`，并渲染中文 `{{AGENTS_PROFILE_SECTION}}`。后续 StorySpec 统一到通用 agent contract，已删除旧的 `templates/AGENTS.codex.md` 专用兼容模板。
- `templates/commands/tasks.md` 和 `templates/commands/write.md` 增加任务边界字段，避免 Codex 在规划阶段误写正文。
- `src/application/export-task-board.ts` 复用任务解析与 artifact scanner，把任务状态、上下文边界、验收标准和 issue 草稿集中为 `task-board.json`。
- `src/application/generate-handoff.ts` 复用任务解析与 artifact scanner，把当前章节、下一任务、必须读取文件、允许修改文件、风险边界和 blocker 集中为 `handoff.md`。
...
- 运行 `node dist/cli.js handoff --json` 做断点续写上下文包冒烟验证。
- 运行 `node dist/cli.js tasks:board --json` 做任务看板导出冒烟验证。
