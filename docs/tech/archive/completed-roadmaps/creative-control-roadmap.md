...
Completed。本文档归档创作控制权体验优化的开发路线。Batch D0-D10 已完成；后续增强需新建专题路线，并回到 [todo-index.md](../../todo-index.md) 登记。
...
- [x] 后续迁移项已关闭：`/novel-clarify` 当前继续使用 Markdown 命令模板，CommandSpec 全量迁移不属于本路线完成口径。
...
- `docs/tech/archive/completed-roadmaps/write-preview-gate.md`
- 相关单测和命令 manifest。
...
状态：Completed。已定义故事成熟度阶段，并让 `status` / `validate` 识别 `idea` 与 `interviewing` 早期状态；独立 `status --creative` 留给后续增强。
...
- [x] 后续增强项已由 D7 覆盖：`novel status` 和 `status --json` 已输出创作空间与 creative gaps，不再单独保留 `status --creative` 活跃待办。
  - 输出创作决策缺口。
  - 输出下一步 3 个建议问题。
...
- `docs/tech/archive/completed-roadmaps/story-maturity-model.md`
...
- 新增 `src/domain/story-stage.ts`，定义 `idea/interviewing/specified/planned/tasked/drafting/revising` 阶段。
- artifact scanner 识别 `idea.md`、`clarifications.md`、`clarifications.json`、`candidates.md`。
- `novel status` 默认输出创作阶段、创作缺口和 3 个下一步问题；独立 `--creative` 参数作为后续增强项。
- `novel validate` 对 `idea/interviewing` 不再提示缺 spec/plan/tasks；`specified/planned` 缺后续产物时降为 info。
- 新增 [story-maturity-model.md](story-maturity-model.md) 记录阶段规则。

### 验证记录

- `npx vitest run tests/unit/story-stage.test.ts tests/unit/artifact-scanner.test.ts tests/unit/get-project-status.test.ts tests/unit/validate-project.test.ts`
- `npm run build`

## Batch D5：来源追踪与 Canon 防污染

状态：Completed。已为 WorldFact / CanonFact 增加来源字段，并让 world/canon 检查识别未确认 AI 建议被写成 confirmed 的风险；review finding 留给 D9 漂移检测批次深化。
...
- [x] 在规格模板中加入来源标记规范。
  ...
  - `aiSuggested`
  - `needsClarification`
- [x] 更新 WorldFact / CanonFact 写入规则。
  - AI 推断只能写入 pending 或 propagation debt。
  - 只有用户确认或已有正文证据才能进入 canon facts。
- [x] `novel canon:check` 增加来源检查。
  - 缺 `sourcePaths` 或缺确认状态时 warning/error。
- [x] `novel world:check` 增加 pending 检查。
- [x] `review` 输出“可能被 AI 偷偷定稿的设定”finding。
...
- `WorldFact` / `CanonFact` 增加 `source.confirmedByUser`、`source.aiSuggested`、`source.needsClarification`。
- `world:check` 和 `canon:check` 会对 `status: confirmed` 且 `aiSuggested: true`、`confirmedByUser: false` 的事实输出 warning。
- `templates/world/world-bible.md`、`templates/world/rules.yaml`、`templates/canon/facts.json` 增加来源字段示例。
- `/novel-specify` 增加来源标记与正典防污染说明。
- review 层“可能被 AI 偷偷定稿”的综合 finding 作为 D9 漂移检测的一部分继续深化。

### 验证记录

- `npx vitest run tests/unit/worldbuilding.test.ts tests/unit/inspect-worldbuilding.test.ts tests/unit/prompt-compiler.test.ts tests/unit/init-project.test.ts tests/unit/manage-context-packs.test.ts tests/unit/validate-project.test.ts`
- `npm run build`
- `npm run build:commands`
- `npm run update:command-manifest`
...
状态：Completed。已建立结构化示例分叉模型、题材种子库和澄清/规格命令的示例分叉展示。

目标：系统性生成 2-3 个不同创作方向示例，而不是模板里手写单一示例。

### 待办

- [x] 新增 `ExampleBranch` 模型。
...
- 新增 `src/domain/example-branch.ts`。
- 新增 `templates/clarification/examples/*.yaml`。
- `selectClarificationQuestions` 支持从示例分叉题材包中稳定抽取 2-3 个不同方向。
- `clarify` 和 `specify` 统一使用“可复制示例分叉”表述。
- `clarifications.md` 会把示例分叉与普通示例分开呈现，并强调 `confirmed: false`。
...
状态：Completed。`status`、`handoff` 与 `context:pack` 已共享澄清记录统计，能暴露待确认创作决策和 AI 建议风险。
...
- [x] `status` 增加“创作空间”板块。
  ...
  - AI 建议未确认数量。
  - 下一步建议问题。
- [x] `status --json` 输出结构化 creative gaps。
- [x] `handoff` 包含创作控制权摘要。
  - 哪些内容不能擅自定稿。
  - 下一个 agent 应先问什么。
- [x] `context:pack` 加入 clarification mustRead。

### 实际完成

- 新增 `src/application/creative-control-summary.ts`，统一读取 `clarifications.json` / `clarifications.md`。
- `novel status` 文本输出增加“创作空间”板块，JSON 中输出 `story.creativeControl`。
- `handoff` 增加“创作控制摘要”，列出不能擅自定稿内容和下一个 agent 应先问的问题。
- `context:pack` 将澄清记录加入 mustRead，并把未确认内容写入约束。
...
状态：Completed。已新增 `novel interview` 与 `novel clarify` CLI，支持非 agent 环境下完成创作访谈、写入澄清记录、replay 旧答案并输出可复制 `/novel-specify` handoff prompt。

目标：当用户不想用 slash command，或当前 agent 不适合写入时，CLI 也能完成创作访谈。

### 待办

- [x] 新增 `novel interview [story]`。
  - 从问题库选择问题。
  - 使用 Inquirer 风格的 input/list/checkbox/confirm。
  - 支持跳过、稍后回答、使用示例。
- [x] 新增 `novel clarify [story]` CLI。
  - 非 agent 环境下也能生成澄清记录。
- [x] 支持 replay。
  - 保存到 `stories/<story>/clarifications.json`。
  - 再次运行时显示旧答案并允许修改。
- [x] 输出 agent handoff prompt。
  - CLI 访谈后生成一段可复制到 Codex 的 `/novel-specify` 输入。

### 实际完成

- 新增 `src/application/interview-story.ts`，封装问题选择、答案合并、旧答案 replay、澄清 Markdown 渲染和 handoff prompt 生成。
- 新增 `src/cli/commands/interview.command.ts`，注册 `novel interview [story]` 与 `novel clarify [story]`。
- 交互模式会展示旧答案、问题影响、可复制示例；`single-choice` 使用 list，`multi-choice` 使用 checkbox，`confirm` 使用 confirm，`scale` 使用 list。
- 非交互模式支持 `--premise`、`--answers`、`--use-examples`、`--max-questions`、`--json` 和 `--no-write`。
- replay 时只更新本轮明确答案，旧答案继续保留，未确认内容仍留在澄清记录而非 specification。
...
- `npx vitest run tests/unit/interview-story.test.ts`
- `npx vitest run tests/smoke/interview-cli.test.ts tests/smoke/cli-commands.test.ts`
- `npm run build`
- `npm run build:commands`
...
状态：Completed。已将创作意图 drift 检查接入 `review`，并让 analyze/tasks/review 模板显式处理创作控制权维度；`branch:create` / `branch:promote` 的 what-if 与影响报告已由前序 workbench 能力提供，本批次补充其在模板契约中的引用。

目标：防止计划、任务、正文偏离用户已确认的创作意图。

### 待办

- [x] `review` 增加创作意图 drift 检查。
  - 正文是否引入未确认设定。
  - 感情线是否跳过慢热阶段。
  - 建设流是否变成纯种田。
  - 文明威胁是否过早压垮基调。
- [x] `analyze` 增加“创作控制权”维度。
- [x] `tasks` 生成时追溯到已确认决策。
- [x] `branch:create` 支持 what-if 分支，不污染 main。
- [x] `branch:promote` 前输出创作影响报告。

### 实际完成

- 新增 `src/application/detect-creative-intent-drift.ts`，从 `clarifications.json` 检查未确认 `ai-suggested` 答案和 required 未答主题是否进入 specification、plan、tasks、scene 或正文。
- `reviewProject` 将 `CREATIVE_INTENT_DRIFT_*` 作为 continuity finding 输出，并转为任务草稿，不自动改正文。
- `/novel-review` 模板要求单列“创作意图漂移”，遇到漂移先确认或转任务草稿。
- `/novel-analyze` 增加 `creative-control` 专项维度。
- `/novel-tasks` 要求正文任务追溯到已确认澄清答案，未确认来源只能生成 `[PLAN-ONLY]` 澄清/复核任务。
- 现有 `branch:create` / `branch:promote` 已写入 `branches/*/impact.md` 和 promote checklist，本批次在任务模板中要求引用 branch impact。

### 产物

- `src/application/review-project.ts`
- `src/application/detect-creative-intent-drift.ts`
- `templates/commands/analyze.prompt.md`
- `templates/commands/review.md`
- `templates/commands/tasks.md`
- `tests/unit/review-project.test.ts`
- `tests/unit/detect-creative-intent-drift.test.ts`
...
- `npx vitest run tests/unit/detect-creative-intent-drift.test.ts tests/unit/review-project.test.ts`
- `npm run build`

## Batch D10：文档、教程与用户引导

状态：Completed。已更新 README、用户文档、命令说明和工作流，让新用户从“输入方向 -> 澄清 -> 预览 -> 写入”开始，并明确低信息量输入不会被 AI 接管。
...
- [x] 新增用户文档：`docs/creative-control.md`。
  - 什么是用户已明确。
  - 什么是 AI 建议。
  - 什么是待确认。
  - 如何使用示例分叉。
...
- README 快速开始改为先创建 idea、运行 `novel interview`，再进入 status/validate。
- 新增 [creative-control.md](../../../creative-control.md)，说明 `user-explicit`、`imported`、`ai-suggested`、`confirmed`、示例分叉、replay、写入前预览、漂移检测和旧项目迁移。
- `docs/commands.md` 更新 `/specify`、`/clarify`、`/tasks`、`/analyze` 的创作控制权说明。
- `docs/workflow.md` 将流程改为 idea -> interview/clarify -> specify preview -> plan -> tasks -> write -> review。
- `docs/index.md` 增加创作控制权指南入口。
