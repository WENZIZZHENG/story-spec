...
Completed。本文记录 2026-05-04 在“法术编译纪元”第一章 dogfood 后完成的 StorySpec 流程优化点，目标是减少每章写作后的手工收尾、误报噪音和路径返工。P0、P1、P2 批次已完成，后续新增章节生产体验增强时应回到 `docs/tech/todo-index.md` 新建专题路线。
...
状态：完成（2026-05-04）。已新增 `compile --check`，只检查可编译性、章节列表和统计信息，不写入 `build/`；`compile` JSON 与文本输出新增 `written` 状态。新初始化 Git 项目的 `.gitignore` 默认忽略 `build/`，减少临时生成稿工作区噪音。
...
状态：完成（2026-05-04）。已新增 `tasks:set-status <taskId> [story] --status done|todo`，并抽出 `setWritingTaskStatus` 作为任务状态单一写入口；`task:finish` 与状态命令都会幂等刷新 `task-board.json`，重复运行不再因为 `generatedAt` 产生无意义 diff。`validate` 会在已有 `task-board.json` 与 `tasks.md` 不同步时给出 `storyspec tasks:board <story>` 修复建议。
