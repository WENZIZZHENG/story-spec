...
`tests/fixtures/co-creation/programming-casting-first-round.json` 是《编程施法》的首轮共创脚本契约，用来判断早期体验是否仍然像“作者和 AI 一起创造小说世界”。

这个样例必须覆盖：

- 保存原始灵感，而不是直接扩写成正典。
- 推荐能力、舞台、势力三个入口。
- 每个入口给出候选，并包含吸引力、代价、关系影响、世界影响、后续钩子和确认边界。
- 势力候选包含资源控制、合法性来源、获利者、受损者、第一碰撞场景和关系钩子。
- 用户可以确认、改写、拒绝或稍后决定。
- 首轮输出创作回声和下一步推荐，但禁止直接写完整 `creative-plan.md`。

样例不是《编程施法》的正式正文或最终大纲。它是防回归材料，配套 `validateFirstRoundScript` 和 `tests/unit/co-creation-first-round-script.test.ts` 使用。
