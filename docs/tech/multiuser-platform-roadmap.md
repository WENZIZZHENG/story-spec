...
- 缺口：真实队列、worker 进程、基础重试/取消语义、幂等键、失败分类和 job dashboard 读模型已完成首批底座；仍缺运行日志查询、产物回传、预览结果落库、告警、分布式锁、BullMQ attempts 策略和失败恢复执行器。
- 建议方案：
