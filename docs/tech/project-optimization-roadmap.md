...
Partially completed。P0 写作链路收紧和章节小样预览已通过 `openspec/changes/add-chapter-writing-lane-sample-preview` 实现并归档；P2 状态语义统一和项目回流闭环已通过 `openspec/changes/add-status-resume-lane` 实现并归档；本文继续保留反向拆解增强、文档收口和 P3 关联路线，作为之后拆分和 OpenSpec 激活的前置入口。其中多用户账号与项目隔离已有独立路线，本文只作横向关联，不重复展开权限设计。
