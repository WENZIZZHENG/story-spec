...
3. 让 `outline -> tasks -> scene -> draft -> review` 的写作链路更顺，并在完整正文前加入章节小样预览。
...
- 建议方案：围绕 `outline -> tasks -> scene -> draft -> review` 的顺序，补齐每一步的下一跳提示、阻断原因、可执行命令或 App 行动入口。章节写作入口应优先告诉用户当前卡在大纲、任务、Scene Card、草稿还是 review，而不是只暴露孤立命令。
...
- OpenSpec 输入：建议新建 `tighten-chapter-writing-lane` 或等价 change。proposal 需说明它优先打通章节生产链路，不重做富文本编辑器；design 需定义 CLI 和 App 的下一跳状态模型。
...
- OpenSpec 输入：建议新建 `add-chapter-sample-preview`。proposal 需说明小样与 beat、草稿、正式正文的区别；design 需定义小样状态、确认动作、保存位置或仅会话输出策略、App/CLI 呈现和不会写入 tracking 的边界。
