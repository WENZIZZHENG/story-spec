...
Completed。P0 写作链路收紧和章节小样预览已通过 `openspec/changes/add-chapter-writing-lane-sample-preview` 实现并归档；P2 状态语义统一和项目回流闭环已通过 `openspec/changes/add-status-resume-lane` 实现并归档；P2 反向拆解增强已通过 `openspec/changes/enhance-reference-reverse-development` 实现并归档；P2 文档收口已通过 `openspec/changes/align-doc-fact-boundaries` 实现并归档。多用户账号与项目隔离已有独立 Planned 路线，本文只作横向关联，不重复展开权限设计。

## 背景和目标

StorySpec 当前已经完成本机 Web 工作台第一阶段，核心创作流程和主要文档契约也已基本收口。本文记录本轮已完成的横向体验收口：
...
- 参考资料：`docs/tech/archive/completed-roadmaps/chapter-production-workflow-roadmap.md`、`docs/tech/archive/completed-roadmaps/immersive-drafting-roadmap.md`、`docs/tech/archive/completed-roadmaps/app-multiuser-roadmap.md` 的本机 App 第一阶段记录。
...
- 涉及文件/模块：`README.md`、`docs/tech/todo-index.md`、`docs/tech/todo-archive.md`、`docs/tech/archive/completed-roadmaps/app-multiuser-roadmap.md`、`docs/tech/project-optimization-roadmap.md`、App 相关命令与页面文案。
...
- 参考资料：`docs/tech/archive/completed-roadmaps/app-multiuser-roadmap.md`、当前本机 App 实现、`storyspec next` 和 `storyspec status` 的现有输出。
...
- 类型：文档、事实源、用户承诺
- 状态：Completed，2026-05-07。完成证据见 `changes/2026-05-07-doc-fact-boundaries.md` 和 `openspec/changes/align-doc-fact-boundaries`。
...
- 已实现方案：同步 `docs/commands.md`、`docs/workflow.md`、`docs/index.md` 和 `docs/quickstart.md` 的章节写作链路，把旧“beat 后直接正文”口径修为“约束卡 -> beat -> 章节小样 -> 完整正文 -> 自检”；同时收紧快速入门的多人协作说法，明确账号、云端和实时协作仍是 Planned。
- 涉及文件/模块：`docs/commands.md`、`docs/workflow.md`、`docs/index.md`、`docs/quickstart.md`、`docs/tech/todo-index.md`、`docs/tech/todo-archive.md`、`changes/*.md`。
- 验收标准：用户文档和待办文档之间不再互相打架；README 和 docs 只讲真实可用能力。
...
- 类型：权限、数据隔离、存储、部署
- 状态：已有独立路线 [单人 App 与多用户项目隔离路线图](archive/completed-roadmaps/app-multiuser-roadmap.md)
- 本路线关系：本路线只作为横向优化建议池的关联入口，不重复展开账号模型、授权检查和隔离设计。
- 验收参考：项目默认私有、跨用户访问失败、导出与删除有明确结果。
- 参考资料：`docs/tech/archive/completed-roadmaps/app-multiuser-roadmap.md`
...
3. P2 反向拆解增强已完成。
4. P2 文档收口已完成。
5. 多用户账号与项目隔离按 `docs/tech/archive/completed-roadmaps/app-multiuser-roadmap.md` 独立推进。
