...
| 优先级 | 路线 | 状态 | 覆盖范围 | 下一步 |
| --- | --- | --- | --- | --- |
| P0 | [章节与维护自动化增强路线图](chapter-maintenance-automation-roadmap.md) | Active | 已完成 `task:finish --commit`、验证失败阻断、章节前置约束卡、`docs:finish --commit` 和 `todo:capture`；剩余共享流程 JSON 契约 | 将共享流程 JSON 契约转为 OpenSpec |
