...
3. 下一步为多人平台 P1 实现任务逐项建立 OpenSpec：worker 分布式锁/恢复、OpenHands 产物回传/日志持久化和独立前端项目。
