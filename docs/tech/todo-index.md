...
| 优先级 | 路线 | 状态 | 覆盖范围 | 下一步 |
| --- | --- | --- | --- | --- |
| P0 | [章节与维护自动化增强路线图](chapter-maintenance-automation-roadmap.md) | Active | 已完成 `task:finish --commit`、验证失败阻断、章节前置约束卡和 `docs:finish --commit`；剩余 `todo:capture`、共享流程 JSON 契约 | 优先将 `todo:capture` 转为 OpenSpec |
