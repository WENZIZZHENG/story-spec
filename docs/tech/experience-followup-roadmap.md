...
- 下一次开发入口：先创建 dogfood 记录文档 `docs/tech/dogfood-authoring-continuation-YYYY-MM-DD.md` 或同等专题记录，按“项目、升级命令、入口文件、能否续写、摩擦、复现命令、是否转 OpenSpec”填表；只有出现可复现缺陷时再新建实现 change。
...
- 下一次开发入口：新增 `docs/tech/dogfood-regression-template.md` 或把模板写入本路线附录；字段固定为“场景、命令、输入、预期、实际、阻断等级、复现文件、候选路线、建议验证”。随后用一个真实回归样例填充记录。
...
- 下一次开发入口：先做只读走查记录 `docs/tech/first-run-experience-notes-YYYY-MM-DD.md`，每条路径记录命令、首屏输出是否过密、下一步是否可复制、作者控制权提示是否清楚；将可复现问题按 P0/P1/P2 分流。
...
- 下一次开发入口：新增或复用 smoke fixture，分别跑四类入口并保存摘要；若只是人工走查，也要把输入样例和命令输出摘要写入 `docs/tech/first-run-experience-notes-YYYY-MM-DD.md`。
...
- 下一次开发入口：先运行或复核现有《编程施法》首轮体验 fixture，并补一份 `docs/tech/co-creation-experience-followup-YYYY-MM-DD.md`；每个候选增强必须写明“提升乐趣/降低负担/保护控制权”三者中的哪一个。
