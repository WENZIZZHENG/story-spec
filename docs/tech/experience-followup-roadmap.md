...
Completed。本文已把归档中只留下方向、尚未登记为活跃待办的体验增强入口统一复核完毕。各项 discovery 未发现 P0/P1 阻断缺陷；后续若出现真实复现证据，应按专题新建 roadmap 或 OpenSpec，而不是重开本文。
...
- [x] 状态：完成（2026-05-05）。已新增 `docs/tech/core-input-panel-followup-2026-05-05.md`，复核短灵感、带字段标签资料、Markdown 表格和无标题资料的 `ingest`、`co:create`、`core --missing --json` 输出；相关 3 个测试文件 10 个测试通过，未发现 P0/P1 缺陷，保留素材类型提示、候选未写入摘要和表格行摘要为 P2 观察。
- 类型：共创输入、核心面板、长文吸收
...
- 下一次开发入口：本轮已完成输入样例记录；后续若真实用户反复误解候选未写入或短资料标签，再转独立 OpenSpec。
...
- [x] 状态：完成（2026-05-05）。已新增 `docs/tech/creative-control-followup-2026-05-05.md`，复核 `creative:report --json`、`next`、`clarification:rollback --json`、`preview specify` 和 `apply` 控制权边界；相关 4 个测试文件 48 个测试通过，未发现 P0/P1 缺陷，保留 rollback 文案和 preview 风险摘要为 P2 可读性观察。
- 类型：创作控制权、来源追踪、确认门禁
...
- 下一次开发入口：本轮已完成控制权走查；后续若 rollback 文案或 preview 风险摘要反复造成误解，再转独立 OpenSpec。
...
- [x] 状态：完成（2026-05-05）。已新增 `docs/tech/workbench-worldbuilding-followup-triage-2026-05-05.md`，分流 WorldFact / CanonFact、Entity Graph、Scene Card、VoiceFingerprint、Reviewer Loop、Genre Preset、Context/Draft/Narrative/Dialogue/Branch/Promise、Research/Style/Compile/Feedback 和 GUI 可视化候选；相关 15 个测试文件 53 个测试通过，未发现 P0/P1 缺陷。
- 类型：世界观、工作台、体验研究
...
- 下一次开发入口：本轮已完成分流；后续新类型包、reviewer 权重或 lint 深接入分别回到生态增强或 agent/CI 质量专题；GUI、复杂图谱、EPUB/Pandoc 只作为 P3 研究储备。
...
- 本路线所有 discovery 已完成；未产出需要立即实现的 OpenSpec change。
- 若后续拆出新专题路线，更新 `todo-index.md`。
- 本轮关闭原因和 P2/P3 储备见各 follow-up 记录。
