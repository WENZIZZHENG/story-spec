...
- 缺口：产品定位、用户旅程、信息架构、主导航、页面地图、首批状态语言、本机 Web 首屏开始路径、独立 `apps/web/` shell 首片、登录/权限只读 UI contract 和静态构建链已完成首批落地；仍缺真实账号/团队产品流、协作评论审批 UI、dev server、富文本编辑器和实时协作体验。
- 建议方案：
