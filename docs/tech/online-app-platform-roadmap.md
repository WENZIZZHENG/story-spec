...
| 优先级 | 子路线 | 状态 | 覆盖范围 | 下一步 |
| --- | --- | --- | --- | --- |
| P0 | [平台地基与发布边界路线图](platform-foundation-roadmap.md) | Planned | 依赖/CI 可复现性、命令产物与 compiled runtime 分离、README 事实边界 | 先把 P0-1/P0-2/P0-3 分别转为小 OpenSpec 或小修 |
| P1 | [完整 App 产品体验路线图](app-ux-roadmap.md) | Planned | 产品体验设计、信息架构、首批页面、状态语言、编辑器与工作台体验 | 先产出 P1-0 产品体验设计规格 |
| P1 | [多人平台与 API 契约路线图](multiuser-platform-roadmap.md) | Planned | 角色模型、API contract、真实 PostgreSQL、真实 worker、完整前端架构 | 先做 API contract 前置设计，再接数据库/worker/前端 |
| P1/P2 | [协作写作与正典合并路线图](collaboration-canon-roadmap.md) | Planned | 候选、评论、审批、正典 patch、导入导出、插件/团队模板 | 在平台对象模型清楚后转独立 OpenSpec |
...
1. [platform-foundation-roadmap.md](platform-foundation-roadmap.md) 的 P0-1/P0-2/P0-3。
2. [app-ux-roadmap.md](app-ux-roadmap.md) 的 P1-0 完整 App 产品体验与界面重设计。
3. [multiuser-platform-roadmap.md](multiuser-platform-roadmap.md) 的 API contract 前置设计。
4. 多人平台角色模型、真实 PostgreSQL、真实 worker 和完整前端架构。
5. 协作写作、正典合并、评论审批、导入导出和编辑器能力。
6. 运维、安全、测试、依赖升级和长期研究按真实风险分批推进。
