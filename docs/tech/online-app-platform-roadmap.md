...
- 仍未完成：账号/团队完整产品流、dev server、错误边界、E2E、实时协同编辑、评论/审批 UI、通知、worker 持久化锁接入和部署运维。
...
3. 协作正典 UI、dev server、错误边界和 E2E。
