...
- 验收标准：设计规格覆盖目标用户、核心路径、主导航、页面地图、关键状态、空状态、权限反馈和响应式边界；交付物清单全部可定位；首批页面都有低保真结构、主要状态和入口说明；用户走查场景都能用文字或线框完整走通；一个没读过文档的新用户能在 3 分钟内完成创建故事、找到章节入口、理解候选不会自动写入、知道在哪里审阅并应用结果；P1-4 前端架构任务必须引用本任务产物。
