...
| P1 | [多人平台与 API 契约路线图](multiuser-platform-roadmap.md) | Active | 角色模型、API contract、真实 PostgreSQL、真实 worker、完整前端架构 | 角色模型、API contract、PostgreSQL driver、worker 队列、显式 OpenHands headless executor、worker lease/heartbeat、preview runtime output record 和前端架构契约底座已完成；下一步进入协作正典协议、worker 分布式恢复、runtime output 持久化/API/UI 和生产可靠性 |
...
- 仍未完成：runtime output PostgreSQL 持久化/API/UI、账号/团队完整产品流、独立浏览器端前端项目、实时协同编辑、评论/审批 UI、通知、worker 分布式锁/恢复执行器和部署运维。
...
3. 协作正典 UI、runtime output PostgreSQL 持久化/API/UI、worker 分布式恢复和独立前端项目。
