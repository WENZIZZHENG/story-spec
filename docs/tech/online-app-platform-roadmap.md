...
| P1 | [多人平台与 API 契约路线图](multiuser-platform-roadmap.md) | Active | 角色模型、API contract、真实 PostgreSQL、真实 worker、完整前端架构 | 角色模型、API contract、PostgreSQL driver、worker 队列、显式 OpenHands headless executor、worker lease/heartbeat、stale worker recovery plan、stale job timeout recovery executor、preview runtime output record、runtime output PostgreSQL repository、runtime output 只读 API、runtime output 本机 UI contract、`apps/web/` 独立 shell 和前端架构契约底座已完成；下一步进入协作正典协议、worker 分布式锁和生产可靠性 |
...
- 仍未完成：账号/团队完整产品流、登录/权限 UI、前端构建链、实时协同编辑、评论/审批 UI、通知、worker 分布式锁和部署运维。
...
3. 协作正典 UI、worker 分布式锁、登录/权限 UI 和前端构建链。
