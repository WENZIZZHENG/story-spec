...
- 类型：协作模型、正典保护、版本管理
- 状态：已完成首批协议底座、HTTP 控制面、PostgreSQL repository、只读审阅面板 API、proposal 评论线程 API、项目活动流 API 与真实 apply executor 首批切片（2026-05-16）。`add-collaboration-canon-merge-protocol` 已新增 proposal、comment thread、review decision、apply request、canon patch、version snapshot、内存 repository 和 apply gate；`add-collaboration-canon-api-control-plane` 已把 proposal/review/patch/apply-request mutation 接入 multiuser server、项目权限守卫、readiness 和 audit log；`add-collaboration-canon-postgres-repository` 已新增协作表、migration v2、数据库 repository 和 server wiring；`add-collaboration-canon-review-read-api` 已提供候选、审批、patch、apply request 的只读审阅面板；`add-collaboration-comment-thread-api` 已把 proposal 评论创建/读取接入 HTTP、PostgreSQL migration v3、repository 和 audit log；`add-project-activity-feed-api` 已基于 audit log 暴露项目活动流；`add-collaboration-apply-executor` 已允许 ready apply request 通过明确 patch content 写入项目 dataRoot，并把 proposal/apply request 标记为 applied；后续仍需评论/审批 UI、通知、rollback 执行、部分应用和冲突合并。
...
- 缺口：首批对象模型、HTTP mutation、PostgreSQL repository、只读审阅面板 API、proposal 评论线程 API、项目活动流 API 和真实 apply executor 首批切片已完成；仍缺 draft session、评论/审批 UI、通知、rollback 执行、部分应用和冲突合并。
- 建议方案：
  1. 定义候选/评论/审批/应用的状态机。
...
- 验收标准：两个用户/agent 对同一章节提出候选时不会静默覆盖；apply 请求包含来源、diff、风险、审批人和回滚入口；正式 `specification.md`、`creative-plan.md`、content、tracking 继续受门禁保护。首批底座已覆盖 apply gate、blocked reasons、HTTP mutation、PostgreSQL repository 和明确内容写入的 apply executor；后续验收需要补 rollback 执行、部分应用和复杂冲突合并。
