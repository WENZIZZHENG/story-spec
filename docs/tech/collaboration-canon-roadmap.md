...
- 类型：协作模型、正典保护、版本管理
- 状态：已完成首批协议底座、HTTP 控制面、PostgreSQL repository、只读审阅面板 API、proposal 评论线程 API、项目活动流 API、真实 apply executor、rollback executor 与协作正典审阅 UI contract 首批切片（2026-05-16）。`add-collaboration-canon-merge-protocol` 已新增 proposal、comment thread、review decision、apply request、canon patch、version snapshot、内存 repository 和 apply gate；`add-collaboration-canon-api-control-plane` 已把 proposal/review/patch/apply-request mutation 接入 multiuser server、项目权限守卫、readiness 和 audit log；`add-collaboration-canon-postgres-repository` 已新增协作表、migration v2、数据库 repository 和 server wiring；`add-collaboration-canon-review-read-api` 已提供候选、审批、patch、apply request 的只读审阅面板；`add-collaboration-comment-thread-api` 已把 proposal 评论创建/读取接入 HTTP、PostgreSQL migration v3、repository 和 audit log；`add-project-activity-feed-api` 已基于 audit log 暴露项目活动流；`add-collaboration-apply-executor` 已允许 ready apply request 通过明确 patch content 写入项目 dataRoot，并把 proposal/apply request 标记为 applied；`add-collaboration-rollback-executor` 已允许 applied apply request 通过明确 rollbackContent 写回项目 dataRoot，并把 proposal/apply request 标记为 rolled-back；`add-collaboration-canon-review-ui-slice` 已在完整 App 前端架构和本机 shell 中暴露协作正典审阅台、状态列、端点地图和 apply/rollback 作者确认边界；后续仍需真实独立前端页面、通知、部分应用和冲突合并。
...
- 缺口：首批对象模型、HTTP mutation、PostgreSQL repository、只读审阅面板 API、proposal 评论线程 API、项目活动流 API、真实 apply executor、rollback executor 和本机 shell 协作正典审阅 UI contract 已完成；仍缺 draft session、真实独立前端页面、通知、部分应用和冲突合并。
- 建议方案：
  1. 定义候选/评论/审批/应用的状态机。
...
- 验收标准：两个用户/agent 对同一章节提出候选时不会静默覆盖；apply 请求包含来源、diff、风险、审批人和回滚入口；正式 `specification.md`、`creative-plan.md`、content、tracking 继续受门禁保护。首批底座已覆盖 apply gate、blocked reasons、HTTP mutation、PostgreSQL repository、明确内容写入的 apply executor 和明确 rollbackContent 写入的 rollback executor；后续验收需要补部分应用和复杂冲突合并。
...
- 已有基础：review findings、feedback import/triage、audit log、tasks board、proposal 评论线程 API。
- 缺口：本机 shell 已有协作正典审阅台 contract；仍缺独立前端中的真实评论/审批交互、行内/段落锚点、通知偏好和任务联动。项目活动流 read API 已完成首批底座。
- 建议方案：在后续独立前端中复用 `collaborationCanonReview` contract，把 proposal 评论线程和项目活动流做成真实候选审阅页，再实现通知偏好和任务联动。
