...
- 缺口：首批对象模型、HTTP mutation、PostgreSQL repository、只读审阅面板 API、proposal 评论线程 API、项目活动流 API、真实 apply executor 和 rollback executor 首批切片已完成；仍缺 draft session、评论/审批 UI、通知、部分应用和冲突合并。
- 建议方案：
  1. 定义候选/评论/审批/应用的状态机。
...
- 缺口：缺评论/审批 UI、行内/段落锚点、通知偏好和任务联动；项目活动流 read API 已完成首批底座。
- 建议方案：先把 proposal 评论线程和项目活动流接入完整 App 候选审阅页，再实现通知偏好和任务联动。
