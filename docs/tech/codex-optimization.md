...
- 增加 `novel codex-status`，输出 Codex 接手项目前需要检查的最小状态。
- 在 `novel init --ai codex` 或 `--all` 时生成根目录 `AGENTS.md`，说明流程、读取顺序和写作边界。
...
- `src/utils/codex-status.ts` 读取 `.specify/config.json`、`stories/*`、`spec/tracking/*.json` 和 `git status --short`，默认输出人类可读摘要，`--json` 输出结构化结果。
- `src/utils/project.ts` 扩展 AI 配置检测，纳入 Codex、Copilot、Qwen、OpenCode 等已支持平台。
- `templates/AGENTS.codex.md` 作为新项目模板，由 `src/cli.ts` 在 Codex 目标项目中复制为 `AGENTS.md`。
...
- 运行 `node dist/cli.js codex-status --json` 做 CLI 冒烟验证。
