...
- 默认 agent：`codex`。
- 技术栈：`Vite + React + TypeScript` 前端，`Fastify` 本地 API 服务，`Tailwind CSS + shadcn/ui` UI 基础。
- 存储：第一阶段继续读写本地 StorySpec 项目文件，不引入数据库。
- 安全边界：本地服务只绑定 `127.0.0.1`，使用 session token；API 只能访问用户选择或创建过的项目根目录。
- 视觉方向：纸面档案 + 本机写作控制台，避免营销页和通用 SaaS 模板感。

### 第一阶段拆分

1. `add-local-single-user-app-workbench`：打通 `storyspec app`、项目选择/创建、最近项目、状态 API 和本机安全边界。
2. `add-local-app-intake-and-core`：在工作台中接入一句灵感、长文资料、核心缺口和创作控制权摘要。
3. `add-local-app-outline-task-views`：接入多大纲候选、任务板和预览确认队列。
4. `add-local-app-chapter-entry`：接入章节草稿入口和写后自检，不默认引入完整富文本编辑器。

第一阶段完成前，不推进账号系统、云端数据库、多用户项目隔离或部署成本边界。
...
- 建议方案：
  ...
  3. 用 Vite/React 建立项目选择页和工作台首屏，优先展示状态、缺口和下一步。
