...
1. `add-local-app-chapter-entry`：接入章节草稿入口和写后自检，不默认引入完整富文本编辑器。
2. `add-local-app-preview-apply-lane`：如章节入口需要统一确认体验，可单独补正式 preview / apply 交互队列；不要混进普通读取视图。

### 第一阶段拆分

1. 已完成的 `add-local-single-user-app-workbench` 和 `add-local-app-shell-ui` 只建立本机 App 地基与首屏 shell，不代表完整 App 已完成。
2. 接下来优先 `add-local-app-chapter-entry`，逐步把章节入口迁入页面。
3. 第一阶段本机工作台完成后，再评估是否需要统一 preview / apply lane 或前端框架化。
