...
1. `add-local-app-intake-and-core`：在工作台中接入一句灵感、长文资料吸收、核心缺口和创作控制权摘要。
2. `add-local-app-outline-task-views`：接入多大纲候选、任务板和预览确认队列。
3. `add-local-app-chapter-entry`：接入章节草稿入口和写后自检，不默认引入完整富文本编辑器。

### 第一阶段拆分

1. 已完成的 `add-local-single-user-app-workbench` 和 `add-local-app-shell-ui` 只建立本机 App 地基与首屏 shell，不代表完整 App 已完成。
2. 接下来优先 `add-local-app-intake-and-core`，让页面能承接作者输入和核心创作状态。
3. 再做 `add-local-app-outline-task-views` 和 `add-local-app-chapter-entry`，逐步把大纲、任务、章节入口迁入页面。
