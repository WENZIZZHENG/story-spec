...
本文档详细介绍 StorySpec 提供的所有斜杠命令，包括用法、参数和最佳实践。
...
| AI 平台 | 命令格式 | 示例 |
|---------|----------|------|
| **Claude Code** | `/storyspec.命令名` | `/storyspec.write` |
| **Gemini CLI** | `/storyspec:命令名` | `/storyspec:write` |
...
### `storyspec creative:report [story]`
...
### `storyspec preview specify [story]` / `storyspec apply <preview-id>`

生成 `specification.md` 写入前预览。`apply` 默认只预览，必须加 `--yes` 才会写入正式目标；存在 blocking 风险时会阻止写入。

```bash
storyspec preview specify 法术编译纪元
storyspec apply specify-法术编译纪元-20260503-1234567890
storyspec apply specify-法术编译纪元-20260503-1234567890 --yes
```
...
**使用示例**：
```
/storyspec.constitution 创建创作原则，强调：
1. 人物塑造优先于情节推进
2. 真实性高于戏剧性
3. 细节刻画要具体生动
4. 避免说教和刻意煽情
...
低信息量输入时，`/specify` 应先输出 preview，并把未确认内容标为 `[需要澄清]`。不要让 AI 用一句话创意直接定稿主角、感情对象、文明级威胁真相或结局。
...
**使用示例**：
```
/storyspec.specify 创建一个都市奇幻小说规格：
- 核心概念：现代都市中隐藏的异能者社会
- 主角：25岁程序员，意外觉醒预知能力
...
**用途**：通过创作访谈明确故事规格中的模糊点，并保存澄清记录

**何时使用**：可以在 `/specify` 前使用，也可以在完成初版规格后、制定计划前使用。用户只给题材标签或一句话灵感时，推荐先澄清。

**工作方式**：
1. 先展示“用户已明确 / 需要澄清 / 可复制示例分叉”
2. 每轮最多问 6-10 个关键问题
3. 保存 `stories/<story>/clarifications.json` 和 `clarifications.md`
4. `ai-suggested` 或 `confirmed: false` 不得直接进入正典

**使用示例**：
```
/storyspec.clarify 异界穿越、轻松冒险、编程施法、慢热感情、文明级威胁
```

AI 可能会问：
- "这本小说最想保留的一句话核心创意是什么？"
- "主角开局最重要的性格缺口或能力盲区是什么？"
- "第一阶段最不想让 AI 直接替你定稿的部分是什么？"

终端里也可以使用 CLI 版：

```bash
storyspec interview idea-demo --premise "异界穿越、轻松冒险、编程施法、慢热感情、文明级威胁" --max-questions 6
storyspec clarify idea-demo --answers "core.scope=主角身份、感情对象和文明威胁真相先不要定死。"
```

**最佳实践**：
- 可以回答“稍后决定”，这比让 AI 猜更安全
- 示例分叉只能作为候选，选择或改写确认后才算用户答案
- 完成后运行 `storyspec status` 查看待确认决策
...
- 预估字数帮助控制节奏
- 标注任务之间的依赖关系
- 正文任务要能追溯到已确认澄清答案；未确认来源只能生成 `[PLAN-ONLY]` 澄清或复核任务
...
- 逻辑预警：故事设计中的潜在漏洞
- 创作控制权：未确认 AI 建议、required 未答问题或待确认边界是否被提前写入产物
- 准备评估：是否可以开始写作
...
或强制指定：
```
/storyspec.analyze --type=framework
```
...
或强制指定：
```
/storyspec.analyze --type=content
```
...
**新建故事**：
```
/storyspec.constitution → /storyspec.specify → /storyspec.clarify →
/storyspec.plan → /storyspec.tasks → /storyspec.track-init → /storyspec.write
```

**定期检查**：
```
每章：/storyspec.track
每5-10章：/storyspec.plot-check
关键转折：/storyspec.analyze
```

**问题诊断**：
```
情节问题：/storyspec.plot-check → /storyspec.expert plot
角色问题：/storyspec.relations → /storyspec.expert character
设定问题：/storyspec.world-check → /storyspec.expert world
```
