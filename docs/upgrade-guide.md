...
**命名空间说明**（v0.15.0+）：
- Claude 命令文件使用 `storyspec.*` 前缀（如 `storyspec.specify.md`），避免与其他工具冲突
- Gemini 命令文件使用 `storyspec/` 子目录
- 使用时分别是 `/storyspec.specify`、`/storyspec:write`、`/storyspec-write` 等平台格式
...
# 更新模板文件（框架更新时使用）
storyspec upgrade --templates

# 更新记忆文件（个人语料等）
storyspec upgrade --memory

# 组合使用：更新命令和脚本
storyspec upgrade --commands --scripts
```
...
# 升级 Gemini 平台的命令文件
storyspec upgrade --ai gemini --commands
...
# 跳过备份（快速升级，谨慎使用）
storyspec upgrade --no-backup

# 预览升级内容（不实际修改）
storyspec upgrade --dry-run

# 组合使用：快速无备份升级
storyspec upgrade -y --no-backup
```
...
4. **紧急快速修复**：
   ```bash
   storyspec upgrade --ai claude --commands -y
   ```
   快速更新 Claude 命令，跳过确认。
...
#### 3.1 找到 story-spec-cn 安装位置

```bash
# macOS/Linux
npm list -g story-spec-cn
# 输出示例：/usr/local/lib/node_modules/story-spec-cn

# Windows
npm list -g story-spec-cn
# 输出示例：C:\Users\YourName\AppData\Roaming\npm\node_modules\story-spec-cn
```
...
```bash
# macOS/Linux
cd my-storyspec cp -r /usr/local/lib/node_modules/story-spec-cn/dist/claude/.claude/commands/* .claude/commands/

# Windows (PowerShell)
cd my-novel
Copy-Item -Recurse "C:\Users\YourName\AppData\Roaming\npm\node_modules\story-spec-cn\dist\claude\.claude\commands\*" .claude\commands\
```
...
```bash
# macOS/Linux
cp -r /usr/local/lib/node_modules/story-spec-cn/scripts/* .specify/scripts/

# Windows (PowerShell)
Copy-Item -Recurse "C:\Users\YourName\AppData\Roaming\npm\node_modules\story-spec-cn\scripts\*" .specify\scripts\
```
...
# 检查命名空间前缀（Claude 使用 storyspec.* 前缀）
ls .claude/commands/storyspec.*

# 检查增强版本特征（应该看到 argument-hint, allowed-tools 等字段）
head -20 .claude/commands/storyspec.analyze.md
```
...
StorySpec v0.16.5 支持以下 13 个 AI 编程助手平台的完整升级：
...
# 升级 Gemini CLI
storyspec upgrade --ai gemini

# 升级 Windsurf
storyspec upgrade --ai windsurf

# 升级 GitHub Copilot（包括 .vscode 配置）
storyspec upgrade --ai copilot
```
...
```bash
# 只更新 Claude 的命令文件
storyspec upgrade --ai claude --commands

# 只更新 Gemini 的脚本
storyspec upgrade --ai gemini --scripts

# 更新 Windsurf 的命令和模板
storyspec upgrade --ai windsurf --commands --templates

# 快速更新 Cursor（跳过确认）
storyspec upgrade --ai cursor -y
```

### 平台检测机制

`storyspec upgrade` 会自动检测项目中已安装的 AI 平台：
...
**新增内容**：
- 基于 spec-kit 的构建系统
- 命名空间隔离（Claude: `storyspec.*` 前缀，Gemini: `storyspec/` 子目录，Codex: `storyspec-` 前缀）
...
3. **验证命名空间**：
   ```bash
   # Claude 命令应该有 storyspec.* 前缀
   ls .claude/commands/storyspec.*

   # Gemini 命令应该在 storyspec/ 子目录
...
使用 `storyspec upgrade` 或 `storyspec init` 自动获取增强版本

**验证增强功能**：
```bash
# 检查是否包含增强字段（注意命名空间前缀）
grep "argument-hint" .claude/commands/storyspec.analyze.md
grep "allowed-tools" .claude/commands/storyspec.track.md
grep "model:" .claude/commands/storyspec.write.md
```
...
**A**: 使用 `storyspec upgrade` 或 `storyspec init --here` 会覆盖命令文件。建议：

**v0.15.0+ 升级（带命名空间）**：
1. 先备份：`cp -r .claude .claude.backup`
2. 运行 `storyspec upgrade`
3. 对比差异：`diff .claude.backup/commands/analyze.md .claude/commands/storyspec.analyze.md`
   - 注意：新版本文件名有 `novel.` 前缀
4. 手动合并自定义部分到新文件
...
2. 检查命令文件是否有增强字段（注意命名空间前缀）：
   ```bash
   grep "argument-hint\|allowed-tools\|model:" .claude/commands/storyspec.analyze.md
   ```
3. 如果没有，运行 `storyspec upgrade` 重新升级
...
**A**: 可以。`storyspec upgrade` 会自动创建 `backup/` 目录：
```bash
# 如果升级后有问题，从自动备份恢复
...
- ✅ AI 工具会自动查找 `storyspec.specify.md`、`storyspec.write.md` 等文件
- ✅ 只有文件名有前缀，使用方式完全不变
...
3. **框架更新** - 更新模板和记忆文件：
   ```bash
   storyspec upgrade --templates --memory
   ```
...
grep "创作强化指令" .claude/commands/storyspec.write.md
```
...
cd my-storyspec storyspec upgrade --commands

# 3. 验证新规范
grep "反AI检测写作规范" .claude/commands/storyspec.write.md
```
...
```bash
# 检查是否有新的反 AI 检测规范
grep "30-50%单句成段" .claude/commands/storyspec.write.md

# 检查是否有 AI 高频词黑名单
grep "弥漫着" .claude/commands/storyspec.write.md

# 检查是否移除了错误的"至少3种感官"指令
! grep "至少3种感官体验" .claude/commands/storyspec.write.md && echo "✅ 已移除错误指令"
```
...
### 升级前
- [ ] 升级 npm 包：`npm install -g story-spec-cn@latest`
- [ ] 确认版本：`storyspec -v` （应该是 0.16.5+）
- [ ] 进入项目目录：`cd my-novel`
...
**方法选择**：
- [ ] **推荐**：运行 `storyspec upgrade` （自动备份，升级所有平台）
- [ ] **交互式**：运行 `storyspec upgrade -i` （手动选择更新内容）
- [ ] **选择性**：运行 `storyspec upgrade --commands --scripts` （指定更新内容）
- [ ] **平台指定**：运行 `storyspec upgrade --ai claude` （只升级特定平台）
- [ ] 或选择其他方法（方法 2/3）
...
- [ ] 检查命名空间：`ls .claude/commands/storyspec.*` （应该有 storyspec.* 前缀）
- [ ] 检查增强功能：`grep "argument-hint" .claude/commands/storyspec.*.md`
...
- [ ] 验证创作强化指令：`grep "创作强化指令" .claude/commands/storyspec.write.md`
...
- 🔖 命名空间支持（Claude: storyspec.* 前缀，Gemini: storyspec/ 子目录）
