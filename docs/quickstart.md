...
`storyspec next` 默认只显示一条最推荐、可复制的下一步命令，以及 2 个可选入口。想看完整入口卡时运行 `storyspec next 法术编译纪元 --verbose`；想只看低负担今日玩法时运行 `storyspec next 法术编译纪元 --modes`。
...
`next` 会根据当前故事状态推荐可以复制的下一步命令；加 `--verbose` 可展开主角、伙伴、世界、场景等完整入口卡。
