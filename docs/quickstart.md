...
AI 会先给出章节前置约束卡和 3-6 条 scene beat，确认方向、冲突、人物变化和风险；beat 确认后先输出章节小样，用来确认读感和文风方向；小样确认或改写后再按正文块推进，并在收尾时汇总正文路径、字数、验证和 tracking 待确认项。章节小样默认不写入正式正文、tracking、tasks 或 canon。下面是一段示例正文片段：
...
当前 StorySpec 以本地单人项目为主，没有内置账号、云端项目隔离或实时多人编辑。如果你要和其他人协作，可以先使用 Git 交换本地项目文件：
