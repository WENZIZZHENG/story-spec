...
本文档面向已经用旧版 StorySpec 创建过项目的作者和维护者，说明如何迁移到当前重构后的 CLI、命令产物、脚本 runtime、插件 manifest 和校验体系。
...
```bash
storyspec status
storyspec validate --severity error
git status --short
```
...
cd my-storyspec storyspec upgrade --dry-run
...
`storyspec upgrade --spec` 会更新写作规范和预设，但会跳过用户数据目录：
...
```bash
storyspec plugins add authentic-voice --dry-run
...
```bash
storyspec plugins add authentic-voice --force
```
...
`storyspec upgrade` 默认会创建备份目录。升级失败或结果不符合预期时：

```bash
cd my-storyspec ls backup
```
...
- [ ] `storyspec validate --severity error` 无 error。
- [ ] `storyspec status` 能看到故事、任务、追踪数据和 blocker 摘要。
- [ ] 已安装 agent integration 的命令目录存在，例如 `.specify/commands/`、`.continue/prompts/`、`.codex/prompts/`、`.claude/commands/`、`.gemini/commands/storyspec/`。
- [ ] `.specify/scripts/runtime/` 存在，旧脚本入口仍可调用。
- [ ] 自定义模板和记忆文件未被误覆盖。
- [ ] 试运行一个非破坏性命令，例如 `/storyspec-analyze` 或对应平台的 `/analyze`。
...
```bash
storyspec init migrated-novel --agent codex
```

然后手动复制旧项目的 `stories/`、`spec/tracking/`、`spec/knowledge/` 和必要的 `.specify/memory/` 文件，再运行 `storyspec validate`。
