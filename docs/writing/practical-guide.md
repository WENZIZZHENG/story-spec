...
StorySpec 支持 13 个 AI 平台，根据 v0.15.0 的多平台优化，命令格式分为三类：

| AI 平台 | 命令格式 | 示例 |
|---------|----------|------|
| **Claude Code** | `/storyspec.命令名` | `/storyspec.constitution`, `/storyspec.write` |
| **Gemini CLI** | `/storyspec:命令名` | `/storyspec:constitution`, `/storyspec:write` |
...
- 例如: `/write` 在 Claude 中应输入 `/storyspec.write`，在 Gemini 中应输入 `/storyspec:write`
...
> - Claude Code 添加 `storyspec.` 前缀（如 `/storyspec.write`）
> - Gemini CLI 添加 `storyspec:` 前缀（如 `/storyspec:write`）
> - 其他平台直接使用（如 `/write`）
