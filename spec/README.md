...
- 职责：存放不同写作方法的模板和规范
- 特点：这些文件是"只读参考"，不应被修改
- 示例：`golden-opening.md`（黄金开篇法则）、`three-act/`（三幕结构）
- 升级策略：可以被 `storyspec upgrade` 安全覆盖
...
**查询顺序**：
1. **先查**：`memory/storyspec-constitution.md`（创作宪法 - 最高原则）
...
**查询顺序**：
1. **先查**：`memory/storyspec-constitution.md`（创作原则）
...
**查询顺序（重要！）**：
1. **先查**：`memory/storyspec-constitution.md`（创作宪法）
...
8. **再查**：`spec/knowledge/natural-expression.md`、`spec/knowledge/punctuation-personality.md`、`spec/knowledge/detail-formulas.md`（如有）
...
**查询顺序**：
1. **先查**：`memory/storyspec-constitution.md`（对照宪法检查合规性）
...
2. **创作宪法**（`memory/storyspec-constitution.md`）
   - 项目的最高创作原则
   - 示例："本作品禁止描写暴力场景"
   - 效果：覆盖所有预设规范
...
5. **写作方法预设**（`spec/presets/`）
   - 标准化的写作规范
   - 示例：`golden-opening.md`, `three-act/`
   - 效果：提供基础规范和最佳实践
...
**场景2**：风格参考 vs 自然表达建议
- **冲突**：`style-reference.md` 说对标作品使用华丽比喻，但 `natural-expression.md` 建议克制修饰
- **解决**：优先 `style-reference.md`，同时避免无功能的修饰堆砌
- **依据**：风格参考优先级更高，自然表达建议只服务于作品质感
...
建议的创建顺序：
1. 运行 `/constitution` 创建 `memory/storyspec-constitution.md`
...
创建一个 `~/.story-spec/presets/` 目录，存放你个人总结的写作规范，然后在项目中引用。
...
2. 或者重命名为项目专用文件（如 `my-opening-rules.md`）
