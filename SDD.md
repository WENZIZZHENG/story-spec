...
- 大型路线图不得无限追加。若单个 roadmap 跨多个主题、批次过多、后续开发经常需要整篇读取，应拆成轻量总览文件和按主题或批次范围组织的子路线文件。
- 拆分后的总览文件只保留主线、共通约束、非目标、优先级视图、子路线索引、风险和完成同步规则；不继续堆放大段批次详情。
- 子路线文件必须能独立支撑对应开发，至少保留目标、已有基础、缺口、建议方案、涉及文件/模块、验收标准、参考资料和不做/边界；不得只写“详情见主文档”。
- 拆分或重组待办后，必须更新 `docs/tech/todo-index.md` 或当前活跃入口，说明 agent 应先读总览，再按任务范围读取对应子路线，避免默认读取全部文档。
- 拆分后必须验证完整性：所有批次或任务都有唯一归属，主路线、子路线和 `todo-index.md` 互相可达，未实现能力仍写作待办而不是已实现功能。
