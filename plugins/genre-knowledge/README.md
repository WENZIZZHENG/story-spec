...
Genre Knowledge Plugin 是一个高级插件，为story-spec提供**类型化写作知识**支持。它包含了商业网文创作中经过验证的方法论、套路和技巧，帮助你写出更符合类型特征的爆款小说。
...
在你的 story-spec 项目根目录执行:

```bash
# 安装类型知识库插件
storyspec plugins:add genre-knowledge
```
...
```bash
# 卸载插件
storyspec plugins:remove genre-knowledge
```
...
```bash
# 运行提取命令（需手动实现）
storyspec run plugins/genre-knowledge/commands/extract-knowledge.md
```
