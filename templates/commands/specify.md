...
在创建或更新任何规格文件之前，先判断用户输入是否足以落盘。

如果用户输入主要是题材标签、风格词、偏好组合或边界声明，例如“异界穿越、轻松冒险、编程施法、慢热感情、文明级威胁”，但缺少主角、舞台、核心冲突、关系对象、威胁形态、篇幅目标等关键创作决策：

- 不要立即运行脚本。
- 不要创建或修改 `stories/*/specification.md`。
- 先用对话澄清，并明确分成三栏：
  - **用户已明确**：逐条复述用户已经给出的方向。
  - **需要澄清**：列出足够多但不压迫的关键问题，通常 6-10 个。
  - **可复制示例分叉**：给 2-3 个不同取向的完整回答示例，用户可以直接复制或改写。
- 示例必须体现不同创作自由度，例如“轻小说冒险版”“硬规则推演版”“先不定主角、由 AI 提问版”。
...
- 只有主题或感觉：如“轻松但有大危机”“想写成长和协作”。
- 缺少可执行的故事核心：不知道谁在什么处境下面对什么冲突、要付出什么代价。

**澄清问题优先覆盖**：
1. 主角身份、现实职业或前史，以及他/她的成功路线和代价。
2. 核心伙伴或感情线对象的身份、性格和与主角的张力。
3. 异界初始舞台，以及这个舞台如何压迫、诱惑或需要主角。
4. 核心能力的规则硬度、限制、失败代价和爽点来源。
5. 第一卷主要势力与冲突，至少说明谁在为什么利益碰撞。
6. 长线威胁的最早小异常、揭示节奏和最终形态。
7. 阅读承诺、目标读者、文风和作品声音。
8. 篇幅、节奏和明确不想写成的方向。
...
2. 运行 `storyspec creative:report` 查看核心要素成熟度和仍可探索的乐趣点
3. 核心要素足够成熟后运行 `storyspec preview plan`，确认预览后再写入 `creative-plan.md`
```
...
- 如果某条 WorldFact / CanonFact 来自 AI 建议，必须写成：
  ...
  - `status: draft`
- 示例分叉仅供选择或改写，不会自动进入规格、计划、任务或 canon。
...
- **根据层级提示下一步**：
  ...
  - Level 4：提示运行 `/clarify` 和 `storyspec creative:report` 澄清关键决策，再进入 `storyspec preview plan`
