# StorySpec - Gemini CLI 配置

本项目已配置为支持 Google Gemini CLI，提供基于七步方法论的完整小说创作体系。

## 🎯 v0.10.0 七步方法论

StorySpec 采用规格驱动开发（SDD）理念，通过系统化的七步流程创作小说。

## ⚠️ 重要：Gemini CLI 命令格式

**Gemini CLI 使用命名空间前缀** `storyspec:`，所有命令格式为：
...
**原因说明**：
- StorySpec 使用 `storyspec:` 命名空间避免与其他工具（如 spec-kit、OpenSpec）的命令冲突
- Gemini CLI 的子目录会自动转换为冒号命名空间（路径：`.gemini/commands/storyspec/write.toml` → 命令：`/storyspec:write`）

> 📖 **详细命令对照**：查看 [docs/ai-platform-commands.md](../docs/ai-platform-commands.md) 了解所有 AI 平台的命令格式差异

### 七步方法论命令

1. **`/storyspec:constitution`** - 创作宪法（定义核心原则）
2. **`/storyspec:specify`** - 故事规格（明确要创造什么）
3. **`/storyspec:clarify`** - 澄清决策（交互式明确模糊点）
4. **`/storyspec:plan`** - 创作计划（制定技术方案）
5. **`/storyspec:tasks`** - 任务分解（生成可执行清单）
6. **`/storyspec:write`** - 章节写作（执行内容创作）
7. **`/storyspec:analyze`** - 综合验证（全方位质量检查）

### 追踪管理命令

- `/storyspec:plot-check` - 情节逻辑检查
- `/storyspec:world-check` - 世界观一致性检查
- `/storyspec:timeline` - 时间线管理
- `/storyspec:relations` - 人物关系管理
- `/storyspec:track` - 综合进度追踪
- `/storyspec:track-init` - 初始化追踪系统

### 专家模式命令

- `/storyspec:expert` - 激活专家模式获取深度指导

## 使用方式

### 推荐工作流（七步方法论）

在 Gemini CLI 中按照以下顺序使用（**注意 `storyspec:` 前缀**）：
...
- 查看 `docs/` 目录获取详细文档
- 访问项目仓库：https://github.com/wordflowlab/story-spec
...
---
*本项目由 StorySpec 团队开发，专为 AI 驱动的中文小说创作设计。*
