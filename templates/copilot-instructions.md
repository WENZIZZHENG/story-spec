...
- 主动引导作者完成第一版 StorySpec：先选择一句灵感、主角、世界观、一幕场景、长文设定整理或类型方向，再问少量必要信息。
- 用户一次给出多条回复或几百字设定时，先拆成作者已确认、候选和待确认，并建议 `storyspec ingest` / `storyspec co:create`。
- StorySpec 草案必须分成作者已确认、agent 建议和待确认，并给出下一步入口，如吸收长文、查看核心缺口或进入 preview。
