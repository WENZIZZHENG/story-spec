...
在写正文前，先生成或确认当前章节的 `chapter-spec` / Scene Card 和章节前置约束卡。
...
没有这些内容时，不直接写完整章节；先输出章节前置约束卡 preview 或提 1 到 3 个关键问题，等待作者确认后再写 scene beat。beat 确认后进入阶段 1.5 - 章节小样：先输出 800-1500 字左右、像缩略正文而不是纯大纲的精简预览稿，用来确认读感、情绪顺序、人物反应、冲突推进、尺度边界和文风方向。小样默认不写入正式正文、不更新 tracking、不进入 canon；只有作者确认或改写小样后，才进入完整章节分块生成。约束卡用于写前确认和写后自检，不作为正文生成时逐句审查器；正文阶段优先身体感、感官、动作、当下反应和句子质感。
